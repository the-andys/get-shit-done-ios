...
describe('gsd-planner reachability_check step', () => {
  ...
  test('file stays under 55000 char limit (fork Opção A: TDD+gap-closure inline)', () => {
    content = content || fs.readFileSync(plannerPath, 'utf-8');
    const normalized = content.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    assert.ok(normalized.length < 55000, `File is ${normalized.length} chars, over the 55000 limit (fork Opção A threshold)`);
  });
});
