...
3. **Gray areas by category** — For each relevant category (UI, UX, Behavior, Empty States, Content), identify 1-2 specific ambiguities that would change implementation. **Annotate with code context where relevant** (e.g., "You already have a CardView" or "No existing pattern for this").
...
☐ Loading behavior — Infinite scroll or pagination?
  (You chose infinite scroll in Phase 4. AsyncStream pagination already set up.)
```
